...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        } elseif ($azione === 'conferma_ritiro_presenza') {
            // Conferma che l'utente ha ritirato il libro di persona
            $prestito_id = $_POST['prestito_id'];
            
            $stmt = $db->prepare("
                UPDATE prestiti 
                SET check_ritiro_utente = 1
                WHERE id = ? AND check_ritiro_bibliotecario = 1
            ");
            $stmt->execute([$prestito_id]);
            
            logActivity($_SESSION['user_id'], 'ritiro_confermato_presenza', 'prestiti', $prestito_id, 
                'Bibliotecario ha confermato ritiro in presenza');
            
            $message = 'Ritiro confermato! Il prestito è ora attivo.';
        }
                        ...
                        <tbody>
                            <?php foreach ($prestiti_in_attesa_ritiro as $p): ?>
                                <tr>
                                    ...
                                    <td>
                                        <form method="POST" style="display: inline;">
                                            <input type="hidden" name="azione" value="conferma_ritiro_presenza">
                                            <input type="hidden" name="prestito_id" value="<?php echo $p['id']; ?>">
                                            <button type="submit" class="btn btn-success btn-sm" title="Conferma che l'utente ha ritirato il libro">
                                                ✅ Conferma Ritiro
                                            </button>
                                        </form>
                                    </td>
