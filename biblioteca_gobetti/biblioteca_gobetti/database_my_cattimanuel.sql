-- Database Biblioteca Gobetti - Per database esistente "my_cattimanuel"
-- Sistema completo di gestione biblioteca scolastica
-- ATTENZIONE: Questo script NON crea il database, usa quello esistente
...
CREATE TABLE IF NOT EXISTS notifiche_disponibilita (
    id INT AUTO_INCREMENT PRIMARY KEY,
    utente_id INT NOT NULL,
    libro_id INT NULL,
    dispositivo_id INT NULL,
...
-- Password: tutti hanno password 'password123'
...
('3', 'B', '2024-2025', 24),
('4', 'B', '2024-2025', 26)
ON DUPLICATE KEY UPDATE numero_studenti=VALUES(numero_studenti);
...
ON DUPLICATE KEY UPDATE numero_copie=VALUES(numero_copie);
...
-- Elimina view se esistono
DROP VIEW IF EXISTS vista_prestiti_attivi;
DROP VIEW IF EXISTS vista_prenotazioni_attive;
DROP VIEW IF EXISTS vista_utenti_blacklist;
...
-- Elimina trigger se esistono
DROP TRIGGER IF EXISTS after_prenotazione_insert;
DROP TRIGGER IF EXISTS after_prenotazione_update;
DROP TRIGGER IF EXISTS after_prestito_insert;
DROP TRIGGER IF EXISTS after_prestito_update;
...
SELECT 'Database Biblioteca Gobetti installato con successo nel database my_cattimanuel!' as Messaggio;
SELECT CONCAT('Tabelle create: ', COUNT(*), ' tabelle') as Info 
FROM information_schema.tables 
WHERE table_schema = 'my_cattimanuel' 
AND table_name IN ('utenti', 'classi', 'libri', 'dispositivi', 'prenotazioni', 'prestiti', 'blacklist_log', 'notifiche_disponibilita', 'impostazioni', 'log_attivita');
